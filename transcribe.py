...
def on_message(self, msg):
    ...
    if "results" in data:
        if data["results"][0]["final"]:
            ...
            LAST = data
        #prints out the current fragment that we are working on
        print(data['results'][0]['alternatives'][0]['transcript'])
        AB=data['results'][0]['alternatives'][0]['transcript']
        with open("test.txt", 'w') as stdout:
                                     stdout.write( AB)
...
def main():
    ...
    ws.run_forever()
    # last message that comes will be saved into the ( Notes ) variable. And I should SAVE it into FILE.text
